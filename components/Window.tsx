...
export default function Window({title, active, setactive}: {title: string, active: boolean, setactive: Dispatch<SetStateAction<boolean>>}) {
    return (
        ...
        style={{
            ...
            <div id="windowbar" className="bg-[rgba(55,55,55,1)] pl-2.5 bg-clip-border w-full flex justify-between items-center rounded-t-lg border-gray h-8 text-white">
                <p>{title}</p>
                <ul className="flex gap-2 pr-5">
                    ...
                    <li><button className="h-full aspect-square text-center hover:bg-red-500" onClick={() => setactive(!active)}>X</button></li>
                </ul>
            </div>
